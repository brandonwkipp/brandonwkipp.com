...
var Timeline = {
    ...
    calculate: function() {
        ...
        if(this.results.length < 4) {
            ...
            this.tempo = Math.floor(60000 / (this.tapAvg / this.results.length));
        }else {
            ...
            this.tempo = Math.floor(60000 / (this.tapAvg / 4));
        }
        this.updateTempo();
    },
    ...
    updateTempo: function() {
        $('#bpm').val(this.tempo.toFixed(4));
        collection.tempo = this.tempo.toFixed(4);

        if(collection.tracks.length > 0) collection.updateTrackEffects();
    }
}
